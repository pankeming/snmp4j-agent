 ...
 * can be changed programatically (see {@link MutableVACM}) or via SNMP but an initial configuration must be created
 * programatically in order to allow any access to the agent via SNMP.
 *
 * @author Frank Fock
 * @version 3.0
 */
public class VacmMIB implements MOGroup, MutableVACM {
    ...
    private void createVacmContextTable() {
        ...
        MOColumn<?>[] vacmContextColumns = new MOColumn<?>[]{
                new MOColumn<OctetString>(1, SMIConstants.SYNTAX_OCTET_STRING, MOAccessImpl.ACCESS_READ_ONLY)};
    ...
    private void createVacmAccessTable() {
        MOColumn<?>[] vacmAccessColumns = new MOColumn<?>[]{
                ...
                new StorageType(colVacmAccessStorageType,
                        MOAccessImpl.ACCESS_READ_CREATE,
                        new Integer32(StorageType.nonVolatile), true),
                new RowStatus<DefaultMOMutableRow2PC>(colVacmAccessRowStatus)
        };
    ...
    private void createVacmViewTreeFamilyTable() {
        MOColumn<?>[] vacmViewTreeFamilyColumns = new MOColumn<?>[]{
                ...
                new StorageType(colVacmViewTreeFamilyStorageType,
                        MOAccessImpl.ACCESS_READ_CREATE,
                        new Integer32(StorageType.nonVolatile), true),
                new RowStatus<DefaultMOMutableRow2PC>(colVacmViewTreeFamilyRowStatus)
        };
    ...
    public int isAccessAllowed(OctetString viewName, OID oid) {
        List<MOTableRow> views = getViews(viewName);
        if (views.size() == 0) {
            if (logger.isDebugEnabled()) {
     ...
     *         the security name.
     *
     * @return <code>true</code> when the entry has been removed or <code>false</code>
     ...
     *         the subtree OID.
     * @param mask
     *         the bit mask which, in combination with <code>subtree</code>, defines a family of view subtrees.
     * @param type
