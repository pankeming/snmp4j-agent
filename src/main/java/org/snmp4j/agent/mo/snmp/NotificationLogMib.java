/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - NotificationLogMib.java  
  _## 
...
//--AgentGen END
    ...
    private void createNlmConfigLogEntry(MOFactory moFactory) {
        ...
        nlmConfigLogEntryColumns[idxNlmConfigLogFilterName] =
                new MOMutableColumn<OctetString>(colNlmConfigLogFilterName,
                        SMIConstants.SYNTAX_OCTET_STRING,
        ...
        nlmConfigLogEntryColumns[idxNlmConfigLogEntryStatus] =
                new RowStatus<NlmConfigLogEntryRow>(colNlmConfigLogEntryStatus);
...
//--AgentGen BEGIN=_METHODS
     ...
     *
     * @return 1 if fired notification events are logged only,
    ...
    public <A extends Address> void notificationLogEvent(NotificationLogEvent<A> notificationLogEvent) {
        if (notificationLogEvent.isSubEvent() == (getLogMode() == Snmp4jNotificationLogModeEnum.sent)) {
                        ...
                        if (filterName != null) {
                            if ((profileName.length() == 0) ||
                                ...
                                if ((snmpNotificationMIB.passesFilter(
                                    ...
                                    nlmStatsGlobalNotificationsLogged.getValue().increment();
                                    checkLimits(profileName);
                                }
                            }
                        }
    ...
    private void checkLimits(OctetString profileName) {
        long globalLimit = nlmConfigGlobalEntryLimit.getValue().getValue();
        if (globalLimit > 0) {
            deleteLogRows(globalLimit, null);
        }
        long ageOut = nlmConfigGlobalAgeOut.getValue().getValue();
        if (ageOut > 0) {
            ...
            for (OID index : victims) {
                ...
                nlmStatsGlobalNotificationsBumped.getValue().increment();
            }
        }
        if (profileName != null) {
            ...
            synchronized (nlmLogEntries) {
                for (int i = 0; (i < delta) && (nlmLogEntries.size() > 0); i++) {
                    ...
                    if (firstIndex != null) {
                        ...
                        NlmStatsLogEntryRow statsRow = nlmStatsLogEntryModel.getRow(firstIndex.trim());
                        if (statsRow != null) {
                            statsRow.getNlmStatsLogNotificationsBumped().increment();
                        }
                        nlmStatsGlobalNotificationsBumped.getValue().increment();
                        if (profileName != null) {
                            NlmStatsLogEntryRow profile = nlmStatsLogEntry.getModel().getRow(profileName.toSubIndex(false));
                            if (profile != null) {
...
//--AgentGen END
    ...
    public class SnmpAdminString implements TextualConvention<OctetString> {
        ...
        public MOColumn<OctetString> createColumn(int columnID, int syntax, MOAccess access,
            ...
            MOColumn<OctetString> col = moFactory.createColumn(columnID, syntax, access, defaultValue, mutableInService);
    ...
    public class TAddress implements TextualConvention<OctetString> {
        ...
        public MOColumn<OctetString> createColumn(int columnID, int syntax, MOAccess access,
            ...
            if (col instanceof MOMutableColumn) {
                MOMutableColumn<OctetString> mcol = (MOMutableColumn<OctetString>) col;
                ConstraintsImpl vc = new ConstraintsImpl();
                vc.add(new Constraint(1L, 255L));
                mcol.addMOValueValidationListener(new ValueConstraintValidator(vc));
            }
    ...
    public class TDomain implements TextualConvention<OID> {
        ...
        public MOScalar<OID> createScalar(OID oid, MOAccess access, OID value) {
            MOScalar<OID> scalar = moFactory.createScalar(oid, access, value);
        ...
        public MOColumn<OID> createColumn(int columnID, int syntax, MOAccess access,
            ...
            MOColumn<OID> col = moFactory.createColumn(columnID, syntax, access,
                    defaultValue, mutableInService);
    ...
    public class SnmpEngineID implements TextualConvention<OctetString> {
