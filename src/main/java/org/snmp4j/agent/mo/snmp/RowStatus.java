    ...
    extends MOMutableColumn<Integer32> implements MOChangeListener, MOTableRowListener<R>
{
  ...
  public void prepare(SubRequest subRequest, MOTableRow row, MOTableRow changeSet, int column) {
    ...
    if (subRequest.getStatus().getErrorStatus() == PDU.noError) {
      ...
      if ((newValue == destroy) && (oldValue != notExistant)) {
        Integer32 storageTypeValue = StorageType.getStorageType(row, getTable());
        if ((storageTypeValue != null) && (storageTypeValue.getValue() == StorageType.permanent)) {
          if (logger.isDebugEnabled()) {
            logger.debug(toString()+": Row '"+ row.getIndex() +
                    "' is permanent and cannot be destroyed! ");
          }
          subRequest.getStatus().setErrorStatus(PDU.inconsistentValue);
        }
      }
      else if ((oldValue == notExistant) || (oldValue == notReady) || (oldValue == createAndGo)) {
        switch (newValue) {
          case createAndGo:
          case notInService:
          case active: {
      ...
      RowStatusEvent rowStatusEvent =
          new RowStatusEvent(this, getTable(), row, changeSet, oldValue, newValue, true, subRequest);
