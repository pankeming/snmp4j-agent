...
public class SnmpTargetMIB implements MOGroup, CounterListener, MOTableRowListener<DefaultMOMutableRow2PC> {
     ...
     * @param <A> the Address class supported by the caller.
     * @return
     *    the target definition if an active (RowStatus) row with {@code name} and the specified context attributes
     *    could be found.
     */
    public <A extends Address> Target<A> getTarget(OctetString name, OctetString contextEngineID,
    ...
    public class SnmpTargetAddrEntryRow extends DefaultMOMutableRow2PC {
        ...
        @SuppressWarnings("unchecked")
        public <A extends Address> Target<A> getTarget(OctetString contextEngineID, OctetString contextName,
                                                       A addressType) {
            ...
            if (paramsRow.getValue(idxSnmpTargetParamsMPModel).toInt() == MPv3.ID) {
                ...
                if (t == null) {
                    if (contextEngineID != null) {
                        t = new UserTarget<A>((A) addr, secName, contextEngineID.getValue(),
                ...
                t = new CommunityTarget<A>((A)addr, community);
            }
        ...
        public <A extends Address> CertifiedTarget<A> getTlsTmTarget(A addr, OctetString secName,
                                                                     OID addrIndex,
                                                                     OID paramsIndex, SnmpTlsTmMib myTlsTmMib) {
            CertifiedTarget<A> t = null;
            ...
            if (tlstmParamsEntryRow != null &&
                ...
                t = new TlsX509CertifiedTarget<A>(addr, secName, serverFingerPrint,
