/*_############################################################################
  _## 
  _##  SNMP4J-Agent 3 - AbstractRequest.java  
  _## 
...
/**
 * The <code>AbstractRequest</code> implements common elements of SNMP and AgentX requests and might be also used for
...
public abstract class AbstractRequest<U extends SubRequest<U>, S, R>
    ...
    public int getErrorStatus() {
        initSubRequests();
        if (errorStatus == SnmpConstants.SNMP_ERROR_SUCCESS) {
            for (SubRequest<U> sreq : subrequests) {
    ...
    protected void resetCompletionStatus() {
        initSubRequests();
        for (SubRequest<U> subReq : subrequests) {
